...
export const useFileContext = () => {
    ...
    if (!context) throw new Error("useFileContext must be used within the a File Provider");
    return context;
};
...
export const FileProvider: React.FC<FileProviderProps> = ({ children }) => {
    ...
    const isValidAudio = (file: File) => file.type.startsWith("audio/") || ["mp3", "wav", "ogg"].includes(file.name.split(".").pop() || "");
    const isValidVideo = (file: File) => file.type.startsWith("video/") || ["mp4", "webp", "ogg"].includes(file.name.split(".").pop() || "");
    ...
    const addFiles = (newFiles: FileList | null) => {
        ...
        if (type === "video") {
            if (filesArray.length > 1) {
                setError("Only 1 video file is allowed.");
                return;
            }
            const file = filesArray[0];
            if (file.size > 800 * 1024 * 1024) {
                setError("Video file must be under 800 MB.");
                return;
            }
